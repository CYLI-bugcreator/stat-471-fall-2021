...
```
cp for complexity, which is the alpha parameter, alpha larger, model less complex
split 0 means there is one terminal region
xerror is the cv error, and xstd is the cv standard deviation
...
optimal_tree_info
```
in classification trees, use misclassification error, which is cp
